...
import unalcol.agents.examples.isi2017I.turianos.fourinrow.AgentA;
...
public final class TreeMinimax {
	...
	public TreeMinimax (int[][] board, boolean max, int[] falls, int turn){
		...
		this.enemy = turn == AgentA.WHITE?AgentA.BLACK:AgentA.WHITE;
		this.turn = turn;
	...
	private void generateTree(BoardNode n, int pruneVariable) {
		if (n.score == Integer.MAX_VALUE || n.score == Integer.MIN_VALUE){
			return;
		}else if (n.deep - this.root.deep >= AgentA.Deep ){
			n.score = calcScore(n);
			n.sheet = true;
			return;
		}else{
		...
		for(int j = 0; j < AgentA.dimension; j++){
	...
	private int calcScore(BoardNode n) {
		...
		for(int k = 0; k < AgentA.dimension; k++){
			...
			for (int m = 0; m < 4; m++) {
				if (directions[0]){ // right
					directions[0] = (j+m < AgentA.dimension ) && ( board[i][j+m] == marker);
					len_line[0]++;
				}
				if (directions[1]){ //down-left
					directions[1] = (j-m > 0) && (i+m < AgentA.dimension ) && ( board[i+m][j-m] == marker);
					len_line[1]++;
				}
				if (directions[2]){ //dawn
					directions[2] = (i+m < AgentA.dimension ) && (board[i+m][j] == marker);
					len_line[2]++;
				}
				if (directions[3]){ //down-right
					directions[3] = (j+m < AgentA.dimension ) && (i+m < AgentA.dimension) && (board[i+m][j+m] == marker);
					len_line[3]++;
				}
			}
			for(int m = 0; m < 4; m++){
	...
	private static boolean isFourInRow(BoardNode n, int turn) {  
		...
		for(int k = 0; k < AgentA.dimension; k++){
			if(n.falls[k] + 1>= AgentA.dimension) 
				continue;
			...
			for (int m = 0; m < 4; m++) {
				if (directions[0]){ // right
					directions[0] = (j+m < AgentA.dimension ) && ( board[i][j+m] == turn);
				}
				if (directions[1]){ //down-left
					directions[1] = (j-m > 0) && (i+m < AgentA.dimension ) && ( board[i+m][j-m] == turn);
				}
				if (directions[2]){ //dawn
					directions[2] = (i+m < AgentA.dimension ) && (board[i+m][j] == turn);
				}
				if (directions[3]){ //down-right
					directions[3] = (j+m < AgentA.dimension ) && (i+m < AgentA.dimension) && (board[i+m][j+m] == turn);
				}
			}
	...
	private static int[] cloneFalls(int[] falls) {
		int [] ans = new int[AgentA.dimension];
	...
	public void Update(int j, int turn) {
		...
		for(BoardNode n : this.root.children){
			if (n.falls[j]+1 >= AgentA.dimension) 
				continue;
